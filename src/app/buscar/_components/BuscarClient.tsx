...
// ═══════════════════════════════════════════════════════════════
// HELPER — Build conversation history for Gemini context
// ═══════════════════════════════════════════════════════════════
function buildConversationHistory(messages: ChatMessage[]): ConversationEntry[] {
    return messages
        .filter(m => m.role === 'user' || m.role === 'assistant')
        .filter(m => m.id !== 'welcome-msg-001')
        .slice(-6)
        .map(m => ({
...
export default function BuscarClient() {
    ...
    const handleSendMessage = useCallback(async (text: string) => {
        ...
        setIsLoading(true);
        try {
            const { responseText, results: newResults, appliedFilters, suggestions } =
                await searchWithAI(text.trim(), buildConversationHistory(messages), userLocation);
            ...
            console.error('[BuscarClient] Search error:', error);
            const errorMessage: ChatMessage = {
                id: uuidv4(),
                role: 'assistant',
                content: 'Lo siento, hubo un error procesando tu búsqueda. Por favor intenta de nuevo. 🙏',
                timestamp: new Date(),
                isNew: true,
            };
    ...
    const handleAudioCapture = useCallback(async (base64: string) => {
        console.log('[BuscarClient] Audio capture:', base64.length, 'bytes');
        const vm: ChatMessage = { id: uuidv4(), role: 'user', content: '🎙️ Búsqueda por voz (procesando...)', timestamp: new Date(), isNew: true };
        setMessages(prev => [...prev, vm]);
        setIsLoading(true);
        try {
            // For now, audio is sent as a text placeholder until voice-to-text is integrated
            const { responseText, results: nr, appliedFilters, suggestions } =
                await searchWithAI('Busco propiedades', buildConversationHistory(messages), userLocation);
            const aiMessage: ChatMessage = {
                id: uuidv4(),
                role: 'assistant',
            ...
            console.error('[BuscarClient] Audio search error:', error);
            setMessages(prev => [...prev, {
                id: uuidv4(), role: 'assistant' as const,
                content: 'No pude procesar la búsqueda por voz. Intenta escribir tu consulta. 🙏',
                timestamp: new Date(), isNew: true,
            }]);
        } finally {
            setIsLoading(false);
